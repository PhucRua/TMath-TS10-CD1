...
# Render.com dùng hệ thống tệp tạm thời, thay đổi thư mục lưu trữ
if os.environ.get('RENDER') == 'true':
    # Sử dụng thư mục /tmp trên Render
...
# Tăng thời gian timeout cho các request - Render cho phép thời gian dài hơn
app.config['SEND_FILE_MAX_AGE_DEFAULT'] = 600  # 10 phút
...
def call_gemini_api(original_text: str, gemini_key: str) -> str:
    ...
    try:
        ...
        # Kiểm tra độ dài văn bản và giới hạn nếu cần
        max_text_length = 60000  # Giới hạn độ dài đầu vào
        truncated = False
        ...
        GEMINI_API_URL = (
            ...
            "gemini-2.5-pro-preview-03-25:generateContent?key=" + gemini_key
        )
        ...
        # Tăng thời gian chờ cho Render.com (60 giây kết nối, 300 giây nhận dữ liệu)
        resp = requests.post(GEMINI_API_URL, json=payload, headers=headers, timeout=(60, 300))
            ...
            if "candidates" in data and len(data["candidates"]) > 0:
                ...
                if "content" in candidate and "parts" in candidate["content"]:
                    ...
                    if corrected_text.strip():
                        if truncated:
                            corrected_text += "\n\n[Lưu ý: Văn bản quá dài, chỉ hiển thị kết quả cho phần đầu]"
        ...
        return "Lỗi: Gemini API timeout - Quá thời gian chờ khi gọi API, vui lòng thử lại sau"
    except requests.exceptions.ConnectionError:
        return "Lỗi: Gemini API - Lỗi kết nối mạng, vui lòng kiểm tra kết nối và thử lại"
...
<D>F9lU9JY8HsOsCzPWlfhn7xHtqKn95z1HkcCQSuqZR82BMwWMU8efBONhI6/xTrcy4i7GXrsuozhbBiAO4ujy5qPytdFemLuqjwFTyvllkcOy3Kbe0deczxnPPCwmSMVKsYInByJoBP3JYoyVAj4bvY3UqZJtw+2u/OIOhoBe33k=</D>
    ...
    try:
        ...
        response = requests.get(github_url, timeout=30)  # Tăng timeout khi lấy token
...
if __name__ == '__main__':
    app.run(host='0.0.0.0', port=5000, debug=(os.environ.get('RENDER') != 'true'))
